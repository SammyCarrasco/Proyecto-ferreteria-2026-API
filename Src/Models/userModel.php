...
class userModel extends connectionDB {
    ...
    public function __construct(array $data) {
        ...
        $rolInput = $data['rol'] ?? '';
if (strcasecmp($rolInput, 'Administrador') === 0 || $rolInput === '1') {
    self::$rol = 1; // Envía 1
} else {
    self::$rol = 2; // Envía 2
}
    ...
    final public static function registrarUsuario() {
        if (\App\BD\sql::verificarRegistro('CALL ConsultarEmpleadoIdentidad(:identidad)', [':identidad' => self::getIdentidad()])) {
            ...
            self::setFechaRegistro(date("Y-m-d H:i:s")); // Formato de fecha estándar MySQL (YYYY-MM-DD HH:MM:SS)

            try {
                $con = self::getConnection();
                
                $query = "CALL RegistrarEmpleado(:nombre, :identidad, :correo, :rol, :clave, :confirmaclave, :IDToken, :fecha_registro)";
                ...
                $stmt->execute([
                    ...
                    ':confirmaclave'  => self::getConfirmarclave(), 
        ...
        if($stmt->rowCount() == 0){ 
            ...
            foreach ($stmt as $val) {                 
                if(Security::validatePassword(self::getClave(), $val['clave'])){
                    ...
                    $stmt->closeCursor(); // Limpia la consulta PDO
